...
export class JsonToGltfService {
    ...
    private static createGlbBuffer(gltfDoc: GltfDocument): Buffer {
        ...
        const totalSize = 12 + 8 + jsonBuffer.length + jsonPadding + 8 + binaryData.length + binPadding;
        const glbBuffer = Buffer.alloc(totalSize);

        let offset = 0;
        glbBuffer.writeUInt32LE(this.GLB_MAGIC, offset); offset += 4;
        glbBuffer.writeUInt32LE(this.GLB_VERSION, offset); offset += 4;
        glbBuffer.writeUInt32LE(totalSize, offset); offset += 4;

        glbBuffer.writeUInt32LE(jsonBuffer.length + jsonPadding, offset); offset += 4;
        glbBuffer.writeUInt32LE(this.CHUNK_TYPE_JSON, offset); offset += 4;
        jsonBuffer.copy(glbBuffer, offset); offset += jsonBuffer.length;
        glbBuffer.fill(0x20, offset, offset + jsonPadding); offset += jsonPadding;

        glbBuffer.writeUInt32LE(binaryData.length + binPadding, offset); offset += 4;
        glbBuffer.writeUInt32LE(this.CHUNK_TYPE_BIN, offset); offset += 4;
        ...
        processedMeshes.forEach((mesh, i) => {
            ...
            const pushAccessor = (
                ...
                gltfDoc.accessors.push({
                    ...
                    componentType,
                    count: data.length / (type === 'SCALAR' ? 1 : parseInt(type.replace('VEC', ''))),
                    type,
                    ...(bounds || {})
                });
            ...
            primitive.attributes.POSITION = pushAccessor(mesh.vertices, 'VEC3', this.COMPONENT_TYPES.FLOAT, this.TARGETS.ARRAY_BUFFER);
            if (mesh.normals && mesh.normals.length) {
                primitive.attributes.NORMAL = pushAccessor(mesh.normals, 'VEC3', this.COMPONENT_TYPES.FLOAT, this.TARGETS.ARRAY_BUFFER);
            }
            if (mesh.uvs && mesh.uvs.length) {
                primitive.attributes.TEXCOORD_0 = pushAccessor(mesh.uvs, 'VEC2', this.COMPONENT_TYPES.FLOAT, this.TARGETS.ARRAY_BUFFER);
            }
            if (mesh.colors && mesh.colors.length) {
                primitive.attributes.COLOR_0 = pushAccessor(mesh.colors, 'VEC4', this.COMPONENT_TYPES.FLOAT, this.TARGETS.ARRAY_BUFFER);
            }
    ...
    private static processMesh(mesh: MeshData, scale: number, center?: boolean): MeshData {
        ...
        const normals = mesh.normals?.length ? mesh.normals : this.generateNormals(vertices, mesh.indices);
