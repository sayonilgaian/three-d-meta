...
export class JsonToObjectController {
    static async createObjectFromJson(request: JsonConfigRequest, reply: FastifyReply) {
        try {
            ...
            if (
                !jsonConfig.meshes ||
                !Array.isArray(jsonConfig.meshes) ||
                jsonConfig.meshes.length === 0
            ) {
                return ResponseUtil.error(
                    reply,
                    'Invalid JSON config: meshes array is required',
                    [],
                    400
                )
            }
            ...
            for (const mesh of jsonConfig.meshes) {
                if (!mesh.vertices || !Array.isArray(mesh.vertices) || mesh.vertices.length === 0) {
                    return ResponseUtil.error(
                        reply,
                        'Invalid mesh: vertices array is required',
                        [],
                        400
                    )
                }
                if (!mesh.indices || !Array.isArray(mesh.indices) || mesh.indices.length === 0) {
                    return ResponseUtil.error(
                        reply,
                        'Invalid mesh: indices array is required',
                        [],
                        400
                    )
                }
                if (mesh.vertices.length % 3 !== 0) {
                    return ResponseUtil.error(
                        reply,
                        'Invalid mesh: vertices array length must be divisible by 3',
                        [],
                        400
                    )
                }
            ...
            if (Buffer.isBuffer(result.data)) {
                ...
                if (uploadToCmsData.success) {
                    ResponseUtil.success(
                        reply,
                        uploadToCmsData.data,
                        '3D asset created succesfully!',
                        200
                    )
                } else {
                    ResponseUtil.success(
